...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    if (!dbResponse.ok) {
      jsonResponse(response, dbResponse.status, { ok: false, message: result.message || "Could not publish debate comment." });
      return;
    }
    ...
    jsonResponse(response, 500, { ok: false, message: error.message || "Could not publish debate comment." });
  }
};
