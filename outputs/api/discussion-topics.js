...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    if (!dbResponse.ok) {
      jsonResponse(response, dbResponse.status, { ok: false, message: result.message || "Could not publish discussion." });
      return;
    }

    jsonResponse(response, 200, {
      ok: true,
      message: "Discussion published.",
      topic: {
        id: result[0]?.id || "",
    ...
    jsonResponse(response, 500, { ok: false, message: error.message || "Could not publish discussion." });
  }
};
